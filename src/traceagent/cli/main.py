"""traceagent CLI entrypoint — thin dispatch over package seams.
...
def _split_root(argv: list[str]) -> tuple[list[str], Path]:
    ...
    # TRACEAGENT_REPO and would otherwise resolve a relative root against the
...
def _model_identity(argv: list[str]) -> tuple[str | None, str | None]:
    ...
    first, then the TRACEAGENT_*_MODEL config envs driver-gate reads.
    ...
    from traceagent.gates.driver_gate import GATE_MODEL_ENV, PRODUCER_MODEL_ENV
...
def main(argv: list[str] | None = None) -> int:
    ...
    if not argv:
        print("usage: traceagent <lint|extract|check|impact|gate|repro|attest|verify|export|"
    ...
    if cmd == "lint":
        from traceagent.spec.findings import partition, render_finding
        from traceagent.spec.lint import collect_findings
    ...
    if cmd == "impact":
        ...
        if not rest:
            print("usage: traceagent impact <target> [--base REF] [root]")
            return 2
        target = rest[0]
        ...
        from traceagent.lineage.impact import impact_from_root
    ...
    if cmd == "mutation-bar":
        from traceagent.gates.mutation_bar import evaluate, load_shard, load_waivers
    ...
    if cmd == "baseline":
        ...
        out_dir = root / ".traceagent" / "baseline"
        out_dir.mkdir(parents=True, exist_ok=True)
    ...
    if cmd == "check":
        from traceagent.codegen.gherkin_gen import gherkin_fallback_green
        ...
        led = RunLedger.start(root / ".traceagent" / "runs",
        ...
        ledger_events = RunLedger.load(root / ".traceagent" / "runs", led.run_id).events
        led.close()
        ...
        gh_dir = root / ".traceagent" / "gherkin"
    ...
    if cmd == "driver-gate":
        from traceagent.gates.driver_gate import run_driver_gate
    ...
    if cmd in ("gate", "gate-campaign"):
        ...
        from traceagent.gates.sandbox import place_tests, prepare_sandbox
        ...
        sandbox_dir = Path(_val(argv, "--sandbox") or ".traceagent/sandbox")
        ...
        checkpoint = sandbox_dir / ".traceagent" / "cache" / "mutants.json"
    ...
    if cmd == "repro":
        from traceagent.debug.repro import repro as repro_run

        if not rest:
            print("usage: traceagent repro <run_id> [root]")
            return 2
        ...
        try:
            ...
            if not root.exists():
                ...
                return 2
            result = repro_run(root, root / ".traceagent" / "runs", run_id)
    ...
    if cmd == "attest":
        ...
        from traceagent.attest.dsse import attest_contract
        from traceagent.attest.keys import parse_timestamp

        signer = CryptoSigner.generate_ed25519()
        producer_model, gate_model = _model_identity(argv)
        envelope = attest_contract(root, producer_model=producer_model,
                                   gate_model=gate_model, signer=signer)
        out_dir = root / ".traceagent"
    ...
    if cmd == "export":
        from traceagent.attest.export import export_attestation
    ...
    if cmd == "verify":
        from traceagent.attest import dsse
        from traceagent.attest.export import load_envelope
    ...
    if cmd == "negotiate":
        ...
        from traceagent.debug.ledger import RunLedger
        from traceagent.negotiate.resume import advance_to_validated, resume
        from traceagent.negotiate.sm import DEFAULT_RETRY_BUDGET, IllegalTransition, NegotiationSM
        from traceagent.negotiate.terms import CounterTermsError, parse_counter_terms, terms_digest
        ...
        resumed = resume(root / ".traceagent" / "runs")
        ...
        try:
            advance_to_validated(sm, led,
                                 kill_after=os.environ.get(
                                     "TRACEAGENT_NEGOTIATE_KILL_AFTER"),
                                 terms=terms)
    ...
    if cmd == "task-gate":
        ...
        if phase == "before":
            ...
            print("usage: traceagent task-gate <before|after> --subagent <type> "
                  "--description <text>")
            return 2
