...
from traceagent.gates.l1 import L1Verdict, run_l1
from traceagent.lineage.matrix import coverage_report, list_all_elements, new_unbound_elements
...
def run_l2(root: Path | str, tier: str = "fast", ledger: RunLedger | None = None,
    ...
    if baseline_elements is None:
        warnings.append(
            ...
            "with `traceagent baseline` (the first real extraction) to make "
            "grandfathering live")
        new_unbound = []
    else:
...
def _reverse_elements(root: Path) -> tuple[list[str], list[str]]:
    ...
    if not roots:
        return (["TR-REVERSE-COVERAGE vacuous: contract meta.element_roots is "
                 ...
                 "as complete"], [])
    from traceagent.lineage.extract import LANG, skipped_dir
...
def _load_baseline_data(root: Path):
    ...
    path = root / ".traceagent" / "baseline" / "elements.json"
    if not path.is_file():
        return None
    try:
