...
from traceagent.gates.sandbox import gate_env
...
def _run_pytest_env(sandbox, test_paths, timeout_s, repo_root):
    ...
    TRACEAGENT_REPO pointing at the real repo (location-independent tests)."""
    extra = {"TRACEAGENT_REPO": str(Path(repo_root))} if repo_root is not None else None
...
def parse_results(sandbox: Path):
    ...
    """
    led_file = sandbox / ".traceagent" / "cache" / "mutants.json"
