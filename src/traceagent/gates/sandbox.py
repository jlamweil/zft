...
1. Module under mutation: mirrored into its package layout inside the sandbox
   (tests import `traceagent.spec.lint` from the installed package; a flat copy
...
- stateful dirs (__pycache__, .hypothesis, .pytest_cache, tool caches, .git,
  .traceagent) are never copied IN — a copied hypothesis database or verdict
...
IGNORED_STATE = shutil.ignore_patterns(
    ...
    ".mypy_cache", ".ruff_cache", ".git", ".traceagent", ".coverage*",
)
