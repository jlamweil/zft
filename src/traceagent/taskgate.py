...
(`[ungated: reason]` or a truthy ZFT_ALLOW_UNGATED; legacy
TRACEAGENT_ALLOW_UNGATED still honored as a fallback) allows a writer
...
def _env_override() -> str | None:
    ...
    The legacy TRACEAGENT_ALLOW_UNGATED remains a transitional fallback when the
    new variable is unset.
    """
    val = os.environ.get("ZFT_ALLOW_UNGATED", "").strip().lower()
    if not val:
        val = os.environ.get("TRACEAGENT_ALLOW_UNGATED", "").strip().lower()
