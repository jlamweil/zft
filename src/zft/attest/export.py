...
Structured views over the attested envelope in ``.zft/attest.json``:
...
from zft.attest.dsse import EnvelopeFormatError, parse_envelope, parse_payload
...
def load_envelope(root: Path | str) -> dict:
    ...
    att_path = Path(root) / ".zft" / "attest.json"
