...
from zft.gates.sandbox import STRIPPED_ENV_VARS
...
PRODUCER_MODEL_ENV = "ZFT_PRODUCER_MODEL"
GATE_MODEL_ENV = "ZFT_GATE_MODEL"
...
def _readonly_child_env():
    ...
    db_dir = tempfile.mkdtemp(prefix="zft-driver-gate-")
    try:
...
def run_driver_gate(root: Path | str, examples_timeout_s: int = 60,
    ...
    ZFT_*_MODEL config envs, else None — never a default name. An
...
def main(argv: list[str] | None = None) -> int:
    ...
    if not argv:
        print("usage: python -m zft.gates.driver_gate <folder>")
        return 2
