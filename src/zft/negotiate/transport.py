...
from zft.negotiate.resume import CANNED_COUNTER_TERMS, recorded_counter_terms, resume
...
class A2aNegotiationServer:
    ...
    def start(self) -> int:
        ...
        resumed_ = resume(self._root / ".zft" / "runs")
        ...
        self._thread = threading.Thread(target=self._http.serve_forever,
                                        name="zft-a2a-transport", daemon=True)
        self._thread.start()
        return self.port
...
def main(argv: list[str] | None = None) -> int:
    parser = argparse.ArgumentParser(
        prog="python -m zft.negotiate.transport",
    ...
    port = server.start()
    print(f"zft-a2a-transport port={port} task={server.task_id} "
