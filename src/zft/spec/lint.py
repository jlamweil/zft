...
from zft.spec.findings import DENY, WARN, Finding, render_finding
...
def collect_findings(root: Path | str) -> list[Finding]:
    ...
    if not spec_dir.exists():
        return [Finding(
            ...
            hint="seed the store with `zft create --alias <ALIAS>`")]
    for path in sorted(spec_dir.rglob("*.json")):
        ...
        try:
            ...
            findings.append(Finding(
                ...
                msg=str(e),
                hint="fix the node to satisfy the ZFTClauseNode schema"))
            continue
